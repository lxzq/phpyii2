...
<script >
    ...
    var g_oJCrop = null;
    function backList (){
        window.location = "/admin/shop/list";
    }
    function openFile(){
        $("#upfile").click();
    }
...
<div style="margin-left: 8%;margin-top: 1%">
    <div class="row">
        <div class="col-lg-5">
            ...
            <?= $form->field($model, 'name')->textInput(['maxlength'=>100])->label("店铺名称") ?>
            <?= $form->field($model, 'address')->textInput(['maxlength'=>100])->label("店铺地址") ?>
            <?= $form->field($model, 'psword')->textInput(['maxlength'=>50])->label("店铺密码") ?>
            <label>店铺图片</label><br>
            <input type="text"  id="tempImage"  readonly="readonly">
            <input type="hidden"  name="logo" id="videoImage" value="<?=$model["logo"] ?>" >
            ...
            <?= $form ->field($model , 'notes') -> textarea(['rows' => 4,'maxlength'=>150])->label("店铺介绍") ?>
        ...
        <div class="col-lg-7">
            ...
            <?php if(!empty($model["logo"])) {?>
                <img   id="videoimg" class="img-rounded"   src="<?= $model["logo"] ?>">
