...
<div style="margin:1%">
    <script>
        function add(course_id) {
            window.location = "/admin/course-manager/price-manager-edit?course_id=" + course_id ;
        }
        function edit(id, course_id) {
            window.location = "/admin/course-manager/price-manager-edit?id=" + id + "&course_id=" + course_id ;
        }
        function del(id, course_id) {
            if (window.confirm('确定要删除吗?')) {
                window.location = "/admin/course-manager/del-manager-price?id=" + id + "&course_id=" + course_id ;
                return true;
            } else {
                return false;
            }
    ...
    <div class="row">
        <div class="col-lg-11">
            <input type="button" class="btn btn-primary" onclick="add(<?= $course_id ?>)" value="新增">
            ...
            <tr class="active">
                ...
                <td class="col-md-4" align="center">
                    <button class="btn btn-primary"
                            onclick="edit(<?= $info["id"] ?>,<?= $course_id ?>);">编辑
                    </button>

                    <button class="btn btn-danger"
                            onclick="del(<?= $info["id"] ?>,<?= $course_id ?>)">删除
                    </button>
                </td>
            </tr>
            <?php
